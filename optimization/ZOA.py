...
CONFIG = {
    "optimizer": "ZOA",      # Zebra Optimization Algorithm
    "population": 25,
    "iterations": 100,
    "cv": 5,
    "random_state": 42,
...
# =====================================================
# 6. ZEBRA OPTIMIZATION ALGORITHM (ZOA)
# =====================================================
def ZOA(objective, lb, ub, N, T):
    """
    Zebra Optimization Algorithm (ZOA)
    Based on:
    - Foraging behavior (exploration)
    - Predator avoidance (diversification)
    - Leader following (exploitation)
    """
    ...
    for t in range(T):
        ...
        for i in range(N):
            ...
            if r < 0.4:
                ...
                random_zebra = pop[np.random.randint(N)]
                new_pos = pop[i] + np.random.rand(D) * (random_zebra - pop[i])

            elif r < 0.8:
                # 🦓 Follow leader (exploitation)
                new_pos = pop[i] + np.random.rand(D) * (best_pos - pop[i])

            else:
                # 🐆 Predator escape (random diversification)
...
best_params, best_mse, runtime, history = ZOA(
    objective,
    lb,
    ub,
    N=CONFIG["population"],
