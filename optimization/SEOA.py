# --------------------------------------------------------------
#  SEOA + Extra Trees Classifier (ETC)
# --------------------------------------------------------------
import time
import numpy as np
import pandas as pd
...
class SEOA:
    ...
    def optimize(self):
        ...
        for t in range(1, self.max_iter + 1):
            ...
            explosion_strength = 1.0 - progress

            for i in range(self.N):
                r = np.random.rand()

                if r < 0.4:  # Uneven material distribution (local dense search)
                    cloud = np.random.normal(0, 0.15 * explosion_strength, self.dim)
                    new_pos = gbest + cloud * (self.ub - self.lb)

                elif r < 0.7:  # Pulse jet (global jump)
                    jet = np.random.uniform(-1.5, 1.5, self.dim) * explosion_strength
                    new_pos = pop[i] + jet * (self.ub - self.lb)

                else:  # Magnetic field perturbation
                    direction = gbest - pop[i]
                    perturb = np.random.randn(self.dim) * 0.3 * (1 - progress)
                    new_pos = pop[i] + 0.6 * direction + perturb
...
print("Optimising Extra Trees Classifier with SEOA...")
seoa = SEOA(N=30, max_iter=200, lb=lb, ub=ub, dim=dim)
best_hp, best_loss, hp_history, runtime = seoa.optimize()
...
hist_df.to_excel("SEOA_ETC_hyperparameters_history.xlsx", index_label="iteration")
print("\nHistory saved to 'SEOA_ETC_hyperparameters_history.xlsx'")