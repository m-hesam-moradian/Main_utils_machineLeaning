# --------------------------------------------------------------
#  POA + QR / ETR  (Excel loading as you requested)
# --------------------------------------------------------------
import time
import numpy as np
import pandas as pd
...
# --------------------------------------------------------------
#  POA implementation (class) - Perfumer Optimization Algorithm
# --------------------------------------------------------------
class POA:
    def __init__(self, N=30, max_iter=200, lb=None, ub=None, dim=None, use_cv=True):
        ...
        self.dim = dim
        self.use_cv = use_cv
        self.alpha = 0.8   # Exploration factor (broad changes)
        self.beta = 0.2    # Exploitation factor (precise adjustments)
    ...
    def optimize(self):
        ...
        for t in range(1, self.max_iter + 1):
            # Linearly decreasing exploration probability
            ...
            for i in range(self.N):
                ...
                if r < exp_prob:
                    # === Exploration Phase (Broad changes to perfume ingredients) ===
                    # Large random jumps toward better solutions
                    idx = np.random.randint(0, self.N)
                    direction = pop[idx] - pop[i]
                    step = np.random.uniform(0.5, 1.5, self.dim)
                    new_pos = pop[i] + step * direction + np.random.randn(self.dim) * (self.ub - self.lb) * 0.1
                else:
                    # === Exploitation Phase (Small precise adjustments) ===
                    # Fine-tuning around global best
                    delta = (gbest - pop[i]) * np.random.uniform(0.01, 0.2, self.dim)
                    new_pos = pop[i] + delta + np.random.normal(0, 0.05 * (self.ub - self.lb), self.dim)
...
poa = POA(N=30, max_iter=200, lb=lb, ub=ub, dim=dim, use_cv=True)
best_hp, best_rae, hp_history, runtime = poa.optimize()
...
hist_df.to_excel("POA_hyperparameters_history.xlsx", index_label="iteration")
print("\nHyper-parameter history saved to 'POA_hyperparameters_history.xlsx'")