...
# Dollmaker Optimization Algorithm (DOA) implementation
def doa_optimize(objective_func, lb, ub, N=30, T=100):
    """
    Dollmaker Optimization Algorithm for minimizing an objective function.
    Human-inspired metaheuristic mimicking doll-making: exploration (pattern selection & sewing)
    via large changes toward best solution, exploitation (beautifying) via small perturbations.

    Parameters:
    - objective_func: function that takes a 1D array of parameters and returns a scalar to minimize.
    - lb: lower bounds (array of length D)
    - ub: upper bounds (array of length D)
    - N: population size (number of dolls)
    ...
    for t in range(1, T + 1):
        for i in range(N):
            # Phase 1: Exploration (Pattern Selection & Sewing)
            I = np.random.choice([1, 2], size=D)
            r = np.random.rand(D)
            new_pos = population[i] + r * (best_params - I * population[i])
            ...
            # Phase 2: Exploitation (Beautifying Details)
            r = np.random.rand(D)
...
# Optimize SGB using DOA
print("Optimizing SGB with DOA...")
best_params_sgb, best_score_sgb = doa_optimize(
...
# Optimize LGBR using DOA
print("Optimizing LGBR with DOA...")
best_params_lgbr, best_score_lgbr = doa_optimize(
