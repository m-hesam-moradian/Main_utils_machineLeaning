...
# =====================================================
# 2. Osprey Optimization Algorithm (OOA) - based on Arctic Puffin Optimization (APO) from PDF
# =====================================================
def ooa_optimize(objective_func, lb, ub, N=25, T=70):
    """
    Osprey Optimization Algorithm (OOA)
    Inspired by Arctic Puffin behaviors (from PDF):
    ...
    plt.title('Convergence Curve - Osprey Optimization Algorithm')
    plt.legend()
    plt.grid(True)
    plt.show()
...
# Objective function for Quantile Regression (QR)
...
# Objective function for Sparse Bayesian Regression (SBR) - ARDRegression
...
print("\n" + "="*90)
print("OSPREY OPTIMIZATION ALGORITHM (OOA) - 2024")
print("="*90)

# 1. Quantile Regression (QR)
print("\nOptimizing Quantile Regression (QR)...")
best_qr, score_qr = ooa_optimize(objective_qr, lb_qr, ub_qr, N=25, T=60)
...
best_sgb, score_sgb = ooa_optimize(objective_sgb, lb_sgb, ub_sgb, N=25, T=60)
...
best_sbr, score_sbr = ooa_optimize(objective_sbr, lb_sbr, ub_sbr, N=25, T=60)
