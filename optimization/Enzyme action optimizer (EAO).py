# --------------------------------------------------------------
#  EAO + QR / ETR  (Excel loading as you requested)
# --------------------------------------------------------------
import time
import numpy as np
import pandas as pd
...
# --------------------------------------------------------------
#  EAO implementation (class) - Enzyme Action Optimizer
# --------------------------------------------------------------
class EAO:
    def __init__(self, N=30, max_iter=200, lb=None, ub=None, dim=None, use_cv=True):
        ...
        self.dim = dim
        self.use_cv = use_cv
        self.binding_rate = 0.7   # Probability of enzyme-substrate binding (exploration)
        self.catalysis_rate = 0.3 # Probability of catalytic reaction (exploitation)
    ...
    def optimize(self):
        ...
        for t in range(1, self.max_iter + 1):
            # Decrease binding rate over iterations (more exploitation later)
            current_binding_rate = self.binding_rate * (1 - t / self.max_iter)

            for i in range(self.N):
                r = np.random.rand()

                if r < current_binding_rate:
                    # === Enzyme-Substrate Binding Phase (Exploration) ===
                    # Random binding with other solutions
                    idx = np.random.randint(0, self.N)
                    substrate = pop[idx]
                    affinity = np.random.uniform(0.4, 1.2, self.dim)
                    new_pos = pop[i] + affinity * (substrate - pop[i]) + np.random.randn(self.dim) * 0.15
                else:
                    # === Catalytic Reaction Phase (Exploitation) ===
                    # Reaction near the best solution (global best)
                    direction = gbest - pop[i]
                    reaction_strength = np.random.uniform(0.05, 0.4, self.dim)
                    new_pos = pop[i] + reaction_strength * direction + np.random.normal(0, 0.08, self.dim)

                new_pos = np.clip(new_pos, self.lb, self.ub)
                new_fit = self._fitness(new_pos)

                # Product Release: Greedy selection (accept if better)
...
eao = EAO(N=30, max_iter=200, lb=lb, ub=ub, dim=dim, use_cv=True)
best_hp, best_rae, hp_history, runtime = eao.optimize()
...
hist_df.to_excel("EAO_hyperparameters_history.xlsx", index_label="iteration")
print("\nHyper-parameter history saved to 'EAO_hyperparameters_history.xlsx'")