...
summary_df["Rank_by_F1"] = summary_df["Mean F1-Score"].rank(ascending=False, method="dense").astype(int)
summary_df = summary_df.sort_values(by="Mean F1-Score", ascending=False)

print("\n🏆 Model Ranking by Mean F1-Score:")
print(summary_df[["Model", "Mean F1-Score", "Rank_by_F1"]].to_string(index=False, float_format="%.4f"))