...
# --- Define models ---
models = {
    "RR": Ridge(alpha=1.0, random_state=42),  # Ridge Regression
    "KNNR": KNeighborsRegressor(n_neighbors=7, p=2, leaf_size=30),  # KNN Regression
...
for model_name, model in models.items():
    ...
    for fold_index, (train_idx, test_idx) in enumerate(kf.split(X), 1):
        ...
        abs_rel_error = np.abs((y_test - y_pred) / (y_test + 1e-8))
        mard = np.mean(abs_rel_error)
