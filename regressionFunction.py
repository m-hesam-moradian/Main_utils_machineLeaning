...
def getAllMetric(measured, predicted):
    ...
    for i in range(N):
        ...
        # AARD component
        S4[i] = abs(predicted[i] - measured[i]) / abs(measured[i])
    for i in range(N - 1):
        # CP
    ...
    AARD = np.mean(S4) * 100  # Average Absolute Relative Deviation (%)
...
def REC(y_true, y_pred):
    ...
    # Calculating Area Under Curve using simpsonon's rule
