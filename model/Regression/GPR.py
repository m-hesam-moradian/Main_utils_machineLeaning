...
X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=0.2, shuffle=False  # 🚀 keeps row order intact
)
...
def build_counterfactual_results(
    df, model, actionable_vars, increments=[0.10, 0.20, 0.35, 0.50]
):
    results = []

    for idx, row in df.iterrows():
        original_cs = row[target_column]

        for i, inc in enumerate(increments, start=1):
            scenario_name = f"Scenario_{i}_{int(inc*100)}%"
            target_cs = original_cs * (1 + inc)

            # Copy features and modify actionable vars
            x_cf = row.drop(labels=[target_column]).copy()
            for var in actionable_vars:
                if var in x_cf:
                    x_cf[var] = x_cf[var] * (1 + inc * 0.5)

            # Predict counterfactual CS
            x_cf_df = pd.DataFrame([x_cf])
            achieved_cs = model.predict(x_cf_df)[0]

            # Store row
            result_row = {
                "Sample_Index": idx,
                "Original_Completion": original_cs,
                "Scenario": scenario_name,
                "Target_Completion": target_cs,
                "Achieved_Completion": achieved_cs,
                "Completion_Difference": achieved_cs - target_cs,
            }

            # Add Original/Counterfactual/Change for each actionable variable
            for var in actionable_vars:
                if var in row:
                    result_row[f"Original_{var}"] = row[var]
                    result_row[f"Counterfactual_{var}"] = x_cf[var]
                    result_row[f"Change_{var}"] = x_cf[var] - row[var]
...
cf_results_df = build_counterfactual_results(df_new, model, actionable_vars)
print(cf_results_df.head())
